...
export default function ProductPage({ product }: { product: ShopProduct }) {
  ...
  return <main className="pdp">
    ...
    <div className="pdp-shell">
      ...
      <section className="pdp-main" aria-labelledby="product-title">
        ...
        <div className="pdp-info">
          ...
          <div className="pdp-fit-note"><p>{apparel ? 'Relaxed silhouette. Find your fit in the size guide.' : 'One size. See product details below.'}</p></div>
          <div ref={purchaseArea} className="pdp-purchase">
            ...
            <button type="button" className="shop-buy-now" onClick={() => purchase(true)}>Buy now</button>
          </div>
          <p className="pdp-checkout-note">Shipping & taxes calculated at checkout.</p>
          <div className="pdp-benefits"><div><span>From a real<br />family-owned garage</span></div><div><b>◎</b><span>Original Carroll’s<br />badge artwork</span></div><div><span>Fit information<br />before you buy</span></div></div>
    ...
    {sticky && <div className="pdp-sticky-cart"><Image src={product.image} alt="" width={48} height={48} /><div><strong>{product.name}</strong><span>{money(product.price)} · {size || 'Choose your size'}</span></div><button type="button" className="shop-primary" onClick={() => purchase()}>{size ? 'Add to cart' : 'Choose size'}</button></div>}
